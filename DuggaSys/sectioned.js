    ...
    if (retdata['versions'].length > 0) {
      for ( j = 0; j < retdata['versions'].length; j++) {
		...
		if(data['writeaccess']) {
			...
			str+="<td style='width:112px;'><input type='button' value='Mark' class='submit-button' title='Edit student results' onclick='changeURL(\"markDugga.php?cid="+querystring['courseid']+"&coursevers="+querystring['coursevers']+"\")' /></td>";
      ...
      str += "<td><input type='button' value='New Item' class='submit-button' onclick='newItem();'/><td>";
			str+="</tr></table>";
      str += "<input type='button' class='fab' value='+' onclick='newItem();'>";
		}else{
			str+="</tr></table>";
		}
